...
const Mycomplaints = () => {
  ...
  useEffect(() => {
    const fetchComplaints = async () => {
      try {
        ...
        const res = await axios.get("http://localhost:5000/complaintForm/my-complaints", {
  ...
  const handleDeleteUserComplaint = async (id) => {
    try {
      const token = localStorage.getItem("token");
      await axios.delete(`http://localhost:5000/complaintForm/my-complaints-delete/${id}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  return (
    <div className="px-12 pt-28 min-h-screen">
      <div className="border-b pb-4 space-y-4">
        ...
        <p>View the ongoing and resolved complaints to understand how we address user issues effectively.</p>
      </div>
      ...
      {complaints.length === 0 ? (
        ...
        <ul>
          {complaints.map((complaint) => (
            <li key={complaint._id} className="border p-4 my-4 rounded-lg shadow-md bg-gray-100">
              <h3 className="text-lg font-semibold">Type: {complaint.complaintType}</h3>
              <p><strong>Model:</strong> {complaint.model}</p>
              <p><strong>Place:</strong> {complaint.place}, {complaint.district}</p>
                    ...
                    {complaint.proof.map((file, index) => (
                      <div key={index}>
                        {file.match(/\.(jpeg|jpg|png)$/) ? (
                          <img src={file} alt="Proof" className="w-40 h-auto rounded-lg" />
